...
d3.chart.bullet = function() {
	...
	function bullet(g) {
		g.each(function(d, i) {
			...
			var markertext = g.selectAll("text.markertext")
					.data(markerz);

			var markertext_text = percentage ? markerz[0]+'%' : markerz[0];
	...
	bullet.percentage = function(x) {
		if (!arguments.length) return percentage;
		percentage = x;
		return bullet;
	}
